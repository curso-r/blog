...
Para adicionar uma plotly no Shiny, criado a partir da função `ggplotly()` ou da função `plot_ly()`, utilizamos o par de funções `plotly::plotlyOutput()` e `plotly::renderPlotly()`. Na função `renderPlotly()`, basta passarmos um código que retorne um gráfico `plotly`, utilizando os `inputs` para especificar as variáveis.
