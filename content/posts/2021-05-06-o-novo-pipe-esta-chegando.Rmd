...
assim isso melhor do que a sintaxe bastante prolixa que o R básico exigia
(`function(x) x`).
...
lugares (não só no `{purrr}`), permite nomes arbitrários para os argumentos e
...
# A notação continua consistente
...
[NEWS](https://stat.ethz.ch/R-manual/R-devel/doc/html/NEWS.html) da versão a fim
de saber quais são as novidades mais interessantes do 4.1. Fora o novo pipe e o
novo lambda, achei algumas coisas dignas de nota:

- Usar `c()` para combinar fatores agora retorna um fator que combina os níveis
de todos (de forma similar ao `forcats::fct_c()`);
...
`...` (possivelmente simplificando o trabalho do `{tidyselect}`);
...
Se você está muito empolgado para testar as novas funcionalidades, já é possível
